...
- 定制版本：`1.15.4-cooperyx.3`，仅限 Obsidian 桌面端个人使用。
- 保持插件 ID `obsidian-spaced-repetition`，直接读取已有设置和笔记内的调度注释。
- 本次优化：多行问答卡内的高亮作为彩色背景块显示；回归测试确认卡内高亮不额外生成挖空卡，卡外独立高亮保留原来的复习规则。
...
- `问题 / ? / 答案` 格式中的 `==高亮==` 只用于强调，整块仍是一张问答卡。显示沿用 Obsidian 原生 Markdown 高亮解析并增强色块样式，原文和复习计划保持原格式。
...
通用构建和测试优先在家庭 Linux 服务器的独立工作目录执行。本轮服务器连接超时，`.3` 改在 Mac 的隔离 scratch 工作区完成；不使用 GPU、不启动服务器服务。
